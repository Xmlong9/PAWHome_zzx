const { listServiceAppointments } = require("../../../services/petServices")
const { getPetList } = require("../../../services/user")

function getMeta(type) {
  if (type === "beauty") return { title: "美容预约记录" }
  if (type === "medical") return { title: "医疗预约记录" }
  if (type === "foster") return { title: "寄养预约记录" }
  return { title: "预约记录" }
}
...
Page({
  ...
  async onLoad(options) {
    const type = typeof (options && options.type) === "string" ? options.type : "beauty"
  ...
  async loadPets() {
    ...
    const petIndex = 0
    this.setData({ pets, petIndex, currentPet: pets[petIndex] || null })
  },
  ...
  async switchPet() {
    ...
    this.setData(
      { petIndex: next, currentPet: this.data.pets[next] || null },
      async () => {
        await this.refreshAppointments()
      }
    )
  ...
  async refreshAppointments() {
    const petId = this.data.currentPet && this.data.currentPet.id
    if (!petId) {
    ...
    try {
      ...
      const now = Date.now()
      const items = (payload.list || []).map((a) => {
        ...
        return {
          id: a.id,
          show,
          title: (a.offering && a.offering.name) || "服务预约",
      ...
      this.setData({ appointments: items.filter((x) => x.show) })
    } catch (e) {
      this.setData({ appointments: [] })
    }
  },
