...
const { getVaccineCatalog } = require("../../../services/vaccines");
const { buildServiceDateOptions, buildSuccessQuery } = require("../../../utils/serviceBooking");
const { getBaseUrl } = require("../../../config/env");

function getVaccineCategory(offeringName) {
...
Page({
  ...
  async loadPageData() {
    try {
      ...
      const hospitals = ((providerPayload && providerPayload.list) || []).map((item) => ({
        ...item,
        coverImage: toAbsoluteUrl(item.coverImage || "")
      }));
      const selectedPetId = (pets[0] && pets[0].id) || "";
      const selectedHospitalId = (hospitals[0] && hospitals[0].id) || "";
      this.setData({
        pets,
        selectedPetId,
        hospitals,
  ...
  async loadVaccines(providerId) {
    const payload = await getServiceOfferings({ serviceType: "vaccine", providerId });
    const vaccines = ((payload && payload.list) || []).map((item) => ({
      id: item.id,
      name: item.name,
      price: item.price,
      age: item.descList && item.descList[0] ? item.descList[0] : item.summary || "",
      cycle: item.descList && item.descList[1] ? item.descList[1] : "",
      availableDates: item.availableDates || [],
      descList: item.descList || []
    }));
    const selectedVaccineId = (vaccines[0] && vaccines[0].id) || "";
    const selectedPrice = (vaccines[0] && vaccines[0].price) || 0;
    ...
    if (vaccines[0]) {
      ...
      await this.loadSlots(vaccines[0], (vaccines[0].availableDates && vaccines[0].availableDates[0]) || "");
    } else {
      this.setData({
        days: [],
        selectedDay: "",
  ...
  async loadVaccineNames(category) {
    const payload = await getVaccineCatalog(category);
    const vaccineNames = (payload && payload.list) || [];
    this.setData({
      vaccineNames,
      selectedVaccineNameId: (vaccineNames[0] && vaccineNames[0].id) || ""
    });
  },

  async loadSlots(vaccine, dateValue) {
    const days = buildServiceDateOptions((vaccine && vaccine.availableDates) || []);
    const selectedDay = dateValue || (days[0] && days[0].value) || "";
    if (!vaccine || !vaccine.id || !selectedDay) {
      this.setData({
        days,
        selectedDay: "",
        timeSlots: [],
    ...
    const slots = ((payload && payload.list) || []).filter((item) => item.remaining > 0);
    ...
    this.setData({
      ...
      timeSlots: slots,
      slots,
      selectedTime: (firstSlot && firstSlot.timeLabel) || "",
      selectedSlotId: (firstSlot && firstSlot.id) || "",
      selectedPrice: (vaccine && vaccine.price) || 0
    });
  },
  ...
  async selectVaccine(e) {
    const selectedVaccineId = e.currentTarget.dataset.id;
    const vaccine = (this.data.vaccines || []).find((item) => item.id === selectedVaccineId);
    ...
    await this.loadVaccineNames(vaccineCategory);
    await this.loadSlots(vaccine, (vaccine && vaccine.availableDates && vaccine.availableDates[0]) || "");
  },
  ...
  refreshFakeMap() {
    ...
    const mapMarkers = points.map((item) => ({
      ...item,
      active: item.providerId === ((selected && selected.providerId) || "")
    }));
    const updateData = { mapMarkers };
  ...
  async selectDay(e) {
    ...
    const vaccine = (this.data.vaccines || []).find((item) => item.id === this.data.selectedVaccineId);
  ...
  selectTime(e) {
    ...
    this.setData({
      ...
      selectedTime: (slot && slot.timeLabel) || ""
    });
  },
  ...
  async submit() {
    const pet = (this.data.pets || []).find((item) => item.id === this.data.selectedPetId);
    const vaccine = (this.data.vaccines || []).find((item) => item.id === this.data.selectedVaccineId);
    const hospital = (this.data.hospitals || []).find((item) => item.id === this.data.selectedHospitalId);
    const slot = (this.data.slots || []).find((item) => item.id === this.data.selectedSlotId);
    const vaccineName = (this.data.vaccineNames || []).find((item) => item.id === this.data.selectedVaccineNameId);
    ...
    try {
      ...
      const query = buildSuccessQuery({
        ...
        storeName: (appointment && appointment.provider && appointment.provider.name) || hospital.name,
        date: (appointment && appointment.serviceDate) || slot.serviceDate,
        time: (appointment && appointment.timeLabel) || slot.timeLabel
      });
      ...
      wx.showToast({ title: (error && error.message) || "预约失败", icon: "none" });
    } finally {
      wx.hideLoading();
    }
  }
...
function getHospitalPoints(hospitals) {
  const presets = {
    "provider-vaccine-1": { lat: 30.2741, lng: 120.1551 },
    "provider-vaccine-2": { lat: 30.1841, lng: 120.2151 }
  };
