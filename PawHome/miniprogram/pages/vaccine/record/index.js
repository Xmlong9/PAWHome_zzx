...
const { getDewormingRecords, getUpcomingVaccineReminder, getVaccineStatus } = require("../../../services/vaccines");
...
Page({
  ...
  async refreshUpcomingReminder() {
    if (!this.data.currentPet || !this.data.currentPet.id) {
    ...
    try {
      ...
      const reminder = (payload && payload.reminder) || null;
      if (!reminder) {
        this.setData({ upcomingReminder: null, upcomingLabel: "", upcomingSub: "" });
        return;
      }
  ...
  async refreshRecords() {
    ...
    if (activeTab === "deworm") {
      ...
      const records = ((payload && payload.list) || []).map((item) => ({
        id: item.id,
        title: item.title,
    ...
    const payload = await getVaccineStatus({ petId, category: activeTab });
    const records = ((payload && payload.list) || []).map((item) => ({
      id: item.id,
      title: item.name,
