...
- [ ] Task 3: 后端核心业务 API 开发
  ...
  - [ ] SubTask 3.2: 开发用户与宠物模块接口（用户资料管理、宠物信息 CRUD）。
  - [ ] SubTask 3.3: 开发社区模块接口（帖子发布、评论、点赞，含分页与筛选）。
  - [ ] SubTask 3.4: 开发商城与服务模块接口（商品列表、购物车、订单创建与查询、服务预约）。
  - [ ] SubTask 3.5: 对接 Supabase Storage 实现图片/文件上传接口。
